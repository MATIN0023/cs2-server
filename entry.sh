...
## SteamCMD update/login
if [[ "$AUTO_UPDATE" == "false" ]]; then
    echo "AUTO_UPDATE=false, skipping SteamCMD login/update"

    if [[ ! -x "${STEAMAPPDIR}/game/cs2.sh" ]]; then
        echo "ERROR: CS2 server files not found in ${STEAMAPPDIR}"
        echo "Run once with AUTO_UPDATE=true to download the server first."
        exit 1
    fi
else
    MAX_ATTEMPTS=3
